...
//
// ANSWER: Making the struct fields lowercased makes them not directly
// accessible from other packages or from outside the ToDo struct. This promotes
// encapsulation and enforces us to interact with ToDo construct through
// predetermined methods in a controled and safe manner.
type ToDo struct {
	toDoMap    DbMap
	dbFileName string
}
...
func (t *ToDo) DeleteItem(id int) error {
	...
	// Check if the item already exists
	_, exists := t.toDoMap[id]

	if !exists {
		return errors.New("the provided ID was not found in database")
	}
...
func (t *ToDo) GetItem(id int) (ToDoItem, error) {
	...
	if !exists {
		return ToDoItem{}, errors.New("the provided ID was not found in database")
	}
