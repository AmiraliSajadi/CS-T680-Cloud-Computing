...
func NewVoterApi() *VoterApi {
	voterApi := &VoterApi{
		...
		Links: []Link{
			{Rel: "self", Href: "/voters"},         // Link to the list of voters
			{Rel: "add-voter", Href: "/voters"},    // Link to add a new voter
			{Rel: "health-check", Href: "/health"}, // Link to health check endpoint
			// Add more links as needed for other actions/resources
		},
	}
...
func (v *VoterApi) AddVoter(voterID uint, firstName, lastName string) {
	...
	voterLinks := []Link{
		{Rel: "self", Href: fmt.Sprintf("/voters/%d", newVoter.VoterID)},
		{Rel: "update", Href: fmt.Sprintf("/voters/%d", newVoter.VoterID)},
		{Rel: "delete", Href: fmt.Sprintf("/voters/%d", newVoter.VoterID)},
		// Add more links as needed for other actions/resources
	}
	v.Links = append(v.Links, voterLinks...)
}
...
func (v *VoterApi) AddVoterGoodHandler(c *gin.Context) {
	...
	if found {
		...
		voterLinks := []Link{
			{Rel: "self", Href: fmt.Sprintf("/voters/%d", this_voter.VoterID)},
			{Rel: "update", Href: fmt.Sprintf("/voters/%d", this_voter.VoterID)},
			{Rel: "delete", Href: fmt.Sprintf("/voters/%d", this_voter.VoterID)},
		}
		v.Links = append(v.Links, voterLinks...)
	}
}
